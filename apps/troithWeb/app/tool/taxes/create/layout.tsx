...
export default function CreateTaxLayout({ children }: PropsWithChildren) {
  return (
    <>
      <header {...animateBasicMotionOpacity()} className="border-b px-4 pb-4">
        <div className="flex items-center gap-2 h-16">
          <H3>Create Tax</H3>
          <Link className={cn('ml-auto', buttonVariants({ variant: 'ghost', size: 'icon' }))} href="/tool/taxes">
            <X className="w-4 h-4" />
          </Link>
        </div>
        <p className="text-muted-foreground text-sm">Create tax schemes to apply them to items for calculating GST in invoices and challans.</p>
      </header>
      <ResizablePanelGroup autoSaveId="CREATE_TAX_FORM_REZISABLE_FORM" direction="horizontal" className=" h-full w-full">
