    ...
    <div className="rounded-lg border bg-background px-3 py-2 text-sm shadow-md">
      {payload.map((entry, index) => (
        <div key={index} className="flex items-center gap-2">
          {!hideIndicator && <div className="h-2 w-2 rounded-full" style={{ background: entry.color || config[entry.name]?.color }} />}
          {!hideLabel && config[entry.name]?.label && <span className="text-muted-foreground">{config[entry.name].label}</span>}
